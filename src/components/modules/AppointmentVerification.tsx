    ...
    return (
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
        <div className="flex items-center gap-2 text-gray-600">
          ...
          <span className="text-sm">Aucun rendez-vous trouvé pour ce visiteur</span>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-green-50 border border-green-200 rounded-lg p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-medium text-green-800">
          {verificationResult.matchType === 'exact' 
            ? 'Rendez-vous confirmé trouvé' 
            : 'Rendez-vous similaires trouvés'}
        </span>
      </div>
      ...
      <div className="space-y-2">
        {verificationResult.appointments.map(appointment => (
          <div
            key={appointment.id}
            className="bg-white border border-green-200 rounded p-3 cursor-pointer hover:bg-green-50 transition-colors"
